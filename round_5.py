...
class Trader:
    ...
    trader_config = {"STARFRUIT": {"Valentina": (0.3393, 0.18),
                                   "Remy": (-2.1116, 0.40),
                                   "Vladimir": (0.6203, 0.17),
                                   "Vinnie": (0.4116, 0.07),
                                   "Ruby": (-0.3076, 0.28),
                                   "Rhianna": (-0.3579, 0.16),
                                   "Amelia": (-0.2656, 0.25),
                                   "Adam": (-0.2473, 0.11)},
                     "AMETHYSTS": {"Valentina": (0.6617, 0.29),
                                   "Remy": (-2.0727, 0.34),
                                   "Vladimir": (0.4987, 0.27),
                                   "Vinnie": (2.1341, 0.37),
                                   "Ruby": (-0.7001, 0.26),
                                   "Rhianna": (-0.5428, 0.36),
                                   "Amelia": (-0.3949, 0.25),
    ...
    def trader_signal(self, state: TradingState, product: Product) -> float:
        """
        Signal generated from de-anonymized trader data in currency unit for pricing shift

        :rtype: float
        :param state: (TradingState) Trading state object of the timestamp
        :param product: (Product) Trading product to generate signal
        :return: Pricing shift signal from trader data
        """
        configs = self.trader_config[product]
        signal = 0.0  # default signal is neutral
        if product in state.market_trades:
            temp = []
            for trade in state.market_trades[product]:
                # only use the trade from the last timestamp
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        # Round 5: De-anonymized trade data (Only apply to Round 1 products)
        ...
        lr_mm.fair_value += self.trader_signal(state, symbol)  # round 5 trader signal
